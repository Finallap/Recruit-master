<?php
/*
 * Created by wolf on 2015年8月12日 下午5:10:44
 */
...
$update_student_status=$pdo->prepare("update student_status set ifconfirm=:ifconfirm,confirm_time=:confirm_time where student_id=:student_id");
...
if ($arr[basicinfo_fill]==1 && $arr[department_wish_fill]==1 && $arr[moreinfo_fill]==1) {
    $update_student_status->bindValue(':student_id', $student_id);
    $update_student_status->bindValue(':ifconfirm', "1");
    $update_student_status->bindValue(':confirm_time', date("Y-m-d h:i:s",time()));
    $update_student_status->execute();
    $update_student_status->closeCursor();

    $selectStmt_student_status->bindValue(':student_id', $student_id);
    $selectStmt_student_status->execute();
    $checkarr=$selectStmt_student_status->fetch();
    $selectStmt_student_status->closeCursor();
    if ($checkarr[ifconfirm]==1 && $checkarr[confirm_time]!="") {
        echo $display_block_1.$student_id.$display_block_2.$basicinfo_status.$department_wish_status.
        $moreinfo_status.$display_block_3.$confirm_success.$display_block_4;
    }
    else {
        $update_student_status->bindValue(':student_id', $student_id);
        $update_student_status->bindValue(':ifconfirm', "0");
        $update_student_status->bindValue(':confirm_time', "");
        $update_student_status->execute();
        $update_student_status->closeCursor();
        echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
        echo '<script type="text/javascript">alert("对不起，由于服务器繁忙，您的确认操作未成功，请稍后重试！！");location.href="sign-in.php"</script>';
    }
    
}
else {
    echo $display_block_1.$student_id.$display_block_2.$basicinfo_status.$department_wish_status.
    $moreinfo_status.$display_block_3.$confirm_failed.$display_block_4;
}
